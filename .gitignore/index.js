...
module.exports.run = async (bot, message, args) => {
    ...
    if(!DMember) return message.channel.send("Veuillez mettre l'utilisateur pour envoyer le message")
 
    let email = args.join(" ").slice(22);
    if(!email) return message.channel.send("Vous devez mettre le messages pour l'envoie.")
 
    let MSG = new Discord.RichEmbed()
    .setDescription("VOUS AVEZ UN MAIL DU SERVEUR " + ` ${message.guild.name} ` + " | " + `${email}`)
